 ...
 * @author gojomo
 */
public class CrawlSubstats implements Serializable {
     ...
     * @param curi
     */
    public synchronized void tally(ResponseContent rspc, Stage stage) {
    	int statusCode = rspc.getStatusCode();
        switch(stage) {      	
            case SCHEDULED:
                totalScheduled++;
                break;
            case RETRIED:
                if(rspc.getStatusCode()<=0) {
                    fetchNonResponses++;
                }
                break;
            case SUCCEEDED:
                fetchSuccesses++;
                fetchResponses++;
                int size = rspc.getContentBytes().length;
                totalBytes += size;
                successBytes += size;
                
                if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
                    notModifiedBytes += rspc.getContentBytes().length;
                    notModifiedUrls++;
//                } else if (IdenticalDigestDecideRule.hasIdenticalDigest(curi)) {
//                    dupByHashBytes += curi.getContentSize();
//                    dupByHashUrls++;
                } else {
                    novelBytes += rspc.getContentBytes().length;
                    novelUrls++;
                }
...
//                }
            ...
            case FAILED:
                if(statusCode<=0) {
                    ...
                    fetchResponses++;
                    totalBytes += rspc.getContentBytes().length;
                    if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
                        notModifiedBytes += rspc.getContentBytes().length;
                        notModifiedUrls++;
//                    } else if (IdenticalDigestDecideRule.hasIdenticalDigest(curi)) {
