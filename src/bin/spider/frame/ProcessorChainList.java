...
public class ProcessorChainList {
	Logger logger = LoggerFactory.getLogger(ProcessorChainList.class);
     ...
     *         set up properly.
     */
    public ProcessorChainList() throws Exception {
        try {
            addProcessorMap("prefetchs"
            		,SettingsXMLUtil.getProcessorList("prefetchs"));
            addProcessorMap("fetchs"
            		,SettingsXMLUtil.getProcessorList("fetchs"));
            addProcessorMap("extractors"
            		,SettingsXMLUtil.getProcessorList("extractors"));
            addProcessorMap("writers"
            		,SettingsXMLUtil.getProcessorList("writers"));
            addProcessorMap("postprocessors"
            		,SettingsXMLUtil.getProcessorList("postprocessors"));
            logger.info(chainList.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }

        if (processorCount() == 0) {
        	throw new Exception("No processors defined");
        }
    }
     ...
     */
    public void addProcessorMap(String name, List<Processor> processorMap) {
    	if (null==processorMap||processorMap.size()<=0)
			return;
