...
//                "prevent this Processor from running."));
    ...
    public final void process(CrawlURI curi) throws InterruptedException {
        ...
        if (SettingsXMLUtil.isInProcessor(Thread.currentThread().getStackTrace()[0].getClassName())) {
            return;
        }
        //这里尚不清楚具体的逻辑
//        if(rulesAccept(curi)) {
...
//        }
    ...
    //得到Context，包括Uri对象和对应的Settings，再转换成DataContainer(Object)
...
//    	//decisionFor(o)返回的是“PASS”，所以应该恒为ture???
...
//    }
    ...
    public CrawlController getController() {
        return SettingsXMLUtil.getController();
    }

    /**
     * 返回Processor对象。扩容处理，利用发射构造新的Processor对象，再修改XML中的值
     * @param the thread number
     * **/
