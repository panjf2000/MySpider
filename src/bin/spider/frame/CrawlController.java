...
import bin.spider.event.CrawlURIDispositionListener;
...
//   throws InitializationException {
       ...
       SettingsXMLUtil.setController(this);
       //解析XML
       getSettings();
...
//   }
    ...
    */
   public void fireCrawledURINeedRetryEvent(ResponseContent rspc) {
       if (registeredCrawlURIDispositionListener != null) {
           // Then we'll just use that.
           registeredCrawlURIDispositionListener.crawledURINeedRetry(rspc);
           return;
       }
       ...
       if (registeredCrawlURIDispositionListeners != null
           ...
           for (Iterator i = registeredCrawlURIDispositionListeners.iterator();
                   i.hasNext();) {
               ((CrawlURIDispositionListener)i.next()).crawledURINeedRetry(rspc);
           }
       }
   }
    ...
    */
   public void fireCrawledURIFailureEvent(ResponseContent rspc) {
       if (registeredCrawlURIDispositionListener != null) {
           // Then we'll just use that.
           registeredCrawlURIDispositionListener.crawledURIFailure(rspc);
       } else {
           // Go through the list.
           if (registeredCrawlURIDispositionListeners != null
               && registeredCrawlURIDispositionListeners.size() > 0) {
   ...
   /**
    * just used to setup processors and Frontier
    * **/  
   private void setupCrawlModules(){
	   if (this.frontier == null) {
         this.frontier = (Frontier)new QueueFrontier();
         try {
...
//                 "unable to initialize frontier: " + e);
	...
	// Setup processors
     if (processorChains == null) {
         try {
			processorChains = new ProcessorChainList();
		} catch (Exception e) {
...
//   private void setupCrawlModules() throws 
...
//   }
    ...
    */
   public void sendCrawlStateChangeEvent(Object newState, String message) {
       synchronized (this.registeredCrawlStatusListeners) {
           this.state = newState;
...
//       maxBytes > 0 && statistics.totalBytesCrawled() >= maxBytes
       if (maxBytes > 0) {
...
//   }
   ...
   private void setupToePool() {
       ...
       toePool.setSize(SettingsXMLUtil.getMAXTHREADS());
   }
